...
export const mutations = {
  ...
  setTranslation(state, translationsArray) {
    // Loop through the translations received, and add them to the text array of the questions.
    for (let i = 0; i < translationsArray.length; i++) {
      state.list[i].text.push(translationsArray[i]);
    }
  }
};
  ...
  getQuestion({ state, rootState, commit }) {
    ...
    if (rootState.session.firstRun) {
      ...
      if (excludeFromNext.length > 0) {
        ...
        console.log("Nothing special, just get a question");
...
export const getters = {
  activeQuestionText: (state, getters, rootState) => {
    console.log(rootState.copy);
    return state.list[state.activeQuestion].text[rootState.copy.lang];
  }
};
